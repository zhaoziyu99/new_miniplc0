...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseMain() {
		...
		// <常量声明>
		auto cst = analyseConstantDeclaration();
		if (cst.has_value())
			return cst;
		// <变量声明>
		auto vary = analyseVariableDeclaration();
		if (vary.has_value())
			return vary;
		// <语句序列>
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		while (true) {
			...
			// 变量可能没有初始化，仍然需要一次预读
			auto pre = next;  //记录之前的标识符
			next = nextToken();
			if (!next.has_value())
				return {};
			_instructions.emplace_back(Operation::LIT, 0);  //初值置为0
			// '='
			if (next.value().GetType() == TokenType::EQUAL_SIGN){
				...
				if (exp.has_value())
					return exp;
				addVariable(pre.value());
				int32_t index = getIndex(pre.value().GetValueString());
				_instructions.emplace_back(Operation::STO, index);   //此时栈顶存储计算结果
				next = nextToken();
				if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
					return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
			}
			...
			{
				addUninitializedVariable(pre.value());
				continue;
			}
			else
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		{
			out = std::stol(next.value().GetValueString());
		}
		else if(next.value().GetType() == TokenType::PLUS_SIGN || next.value().GetType() == TokenType::MINUS_SIGN)
		{
			auto pre = next; //记录第一个符号
			next = nextToken();
			if(next.value().GetType() != TokenType::UNSIGNED_INTEGER)	
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrConstantNeedValue);
			if (pre.value().GetType() == TokenType::PLUS_SIGN)
			{
				out = std::stol(next.value().GetValueString());
			}
			else if (pre.value().GetType() == TokenType::MINUS_SIGN)
			{
				out = std::stol(next.value().GetValueString());
				out *= -1;
			}
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		// 标识符是常量吗？
		else if (isConstant(next.value().GetValueString()))
		...
		if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
		//从未初始化中移除
		if (isUninitializedVariable(pre.value().GetValueString())) {
			_vars[pre.value().GetValueString()] = _uninitialized_vars[pre.value().GetValueString()];
			_uninitialized_vars.erase(pre.value().GetValueString());
		}
		// 需要生成指令吗？
		_instructions.emplace_back(Operation::STO, getIndex(pre.value().GetValueString()));
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseFactor() {
		...
		switch (next.value().GetType()) {
			...
			case TokenType::IDENTIFIER: {
                if (!isDeclared(next.value().GetValueString()))//未声明,返回编译错误
                    return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNotDeclared);
            ...
            case TokenType::UNSIGNED_INTEGER: {
                _instructions.emplace_back(Operation::LIT, std::stol(next.value().GetValueString()));
                break;
            }
